...
class Impartus:
                    ...
                    if not os.path.exists(enc_stream_filepath) or os.path.getsize(enc_stream_filepath) == 0:
                        ...
                        while not download_flag:
                            ...
                            try:
                                ...
                                headers['cookie'] = f"Bearer={cookie_dict['Bearer']}"
                                headers["connection"] = "keep_alive"
                                content = requests.get(item['url'],headers=self.session.headers, timeout=self.timeouts).content
                                download_flag = True
    ...
    def login(self):
        ...
        self.session.headers.update({'Referer':'http://172.16.3.20/login/'})
        self.session.headers.update({'Accept':'application/json, text/plain, */*'})
        self.session.headers.update({'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:123.0) Gecko/20100101 Firefox/123.0'})
        self.session.headers.update({'Origin':'http://172.16.3.20'})
        self.session.headers.update({'Host':'172.16.3.20'})
        self.session.headers.update({'Content-Length':'75'})
